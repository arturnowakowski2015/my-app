...
const TreeNode = (props) => {
  ...
  props.changeconfig(props.config==0 ? 2 : 1)
  }
  let el1 = {}
  ...
  const makeopacity = (nodes, str, d, id) => {
    ...
    nodes.map((t, i) => {
      ...
      if (t.name == str) {
        ...
        if(t.depth==0){
          elmenu.parentroot.name=t.name
          elmenu.parentroot.id=i-1
        }
  ...
  let y = -1;
  const removefromroot = (tr) => { alert(p+"::"+arr.length)
    y = tr.children.findIndex(obj => obj.opacity==0.4) 
 if(p==0)
    if(elmenu.child.children && elmenu.child.children.length>0 ){
      tr.children.splice(y, 1, elmenu.child.children[0].children[0])
      mode=1;
    }
      else {tr.children.splice(y, 1);mode=1}
   p++;
      makeidlev(tree.children, 0, 0)
      for (let ii = 0; ii < 20; ii++) {
        c = 0;
  ...
  const removeopacity = (tr) => {
    ...
    tr.map((t) => {
      ...
      if (t.name == elmenu.parentold.name) {
        ...
        console.log("ppppppppppppppppppppppppppppppppppppppppppppppppppppp"+JSON.stringify(t))
        y=-1
        mode=1;
      }
  ...
  const removeprobe = (nodes, r, remchild) => {
    ...
    if(remchild=="root")
    {      if(nodes[1]!=undefined && nodes[1].children!=undefined  ) 
  ...
  let m=0
  let str2=""
  const addroot = (nodes, str) => {
    //elmenu.child.children=[]
nodes.map((t) => {
  alert(m+"::"+t.name+"::::"+t.depth)
        t.opacity=1; 
      if(t.depth>0){
        str2=t.name;
        t.name="pppp "+str
  ...
  if(t.children){addroot(t.children, str2);m=0}
}) 
    makeidlev(tree.children, 0, 0)
    for (let ii = 0; ii < 20; ii++) {
      c = 0;
